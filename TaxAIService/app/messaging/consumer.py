...
async def start_rabbitmq_consumer() -> None:
    ...
    Khởi chạy các Consumer: TaxRule, OCR,...
    ...
    while True:
        try:
            ...
            ocr_request_queue = await channel.declare_queue(settings.RABBITMQ_OCR_REQUEST_QUEUE, durable=True)
            await channel.declare_queue(settings.RABBITMQ_OCR_RESPONSE_QUEUE, durable=True)
